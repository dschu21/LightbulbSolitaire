...
public class Try  extends Applet implements ActionListener {
	...
	public void init(){
		...
		high.setFont(new Font("Lucida",Font.PLAIN,200));
		...
		scoreLabel1.setFont(new Font("Arial",Font.BOLD,30));
		scoreLabel2 = new Label("    Hard Level");
		scoreLabel2.setFont(new Font("Arial",Font.BOLD,30));

		for (byte i=0;i<scores1.length;i++){ 
			for (byte n=0; n <scores1[i].length;n++){
				if(n%2!=0)
					scores1[i][n] = new TextField(75);               //should this work?
				else
		...
		for (byte i=0;i<scores2.length;i++){ 
			for (byte n=0; n <scores2[i].length;n++){
				if(n%2!=0)
					scores2[i][n] = new TextField(75);               //should this work?
				else
		...
		try {
			input1 = new BufferedReader (new FileReader ("F:/ICS3U1/programs/Final Project/High Scores/High Scores 1.txt"));
		...
		try {
			input2 = new BufferedReader (new FileReader ("F:/ICS3U1/programs/Final Project/High Scores/High Scores 2.txt"));
		...
		restart = new Button ("Restart same game");
	...
	public void setHighScore() throws IOException { 
		if(level==1){
			output1 = new PrintWriter (new FileWriter ("F:/ICS3U1/programs/Final Project/High Scores/High Scores 1.txt"));
			sort();
		...
		if(level==2){
			output2 = new PrintWriter (new FileWriter ("F:/ICS3U1/programs/Final Project/High Scores/High Scores 2.txt"));
	...
	}	//need to put the 0 score to the bottom of the list
